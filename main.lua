...
local function chunkFor(rel)
  ...
  if not source then
    error(("SOLO_BATTLE_VOXEL: %s is missing -- reinstall the mod"):format(rel), 0)
  end
  local chunk, err = load(source, "@" .. mod.path .. "/" .. rel)
  if not chunk then
    error(("SOLO_BATTLE_VOXEL: %s did not compile: %s"):format(rel, tostring(err)), 0)
  end
  return chunk
end
...
local RunInfo = V.require("RunInfo")
...
local SETTINGS = {
  ...
  { UiBackplates.hudColor,
    ...
    when = function() return stagedBattles() end, full = true },
  { UiBackplates.hudStyle,
    "How the battle HUD is drawn. OG is the engine's own status block, "
    .. "recoloured and snapped to the window corner. PANEL replaces it with "
    .. "a black box drawn for this build's small screen: the name across the "
    .. "full width of the box at twice the size OG can reach, the level (or "
    .. "the status, which replaces it exactly as the ROM does), the enemy "
    .. "party balls, and an HP bar sized to be read at a glance. PANEL does "
    .. "not draw the intro pokeball reveal or the Safari ball count; its own "
    .. "ball cluster shows the same party state for the whole battle. HUD "
    .. "SCALE has no effect under PANEL, which sets its own size.",
    when = function() return stagedBattles() end, full = true },
  { UiBackplates.arenaFill,
    "WHITE draws a solid white layer in front of the voxel world: the "
    .. "normal battle camera is retained, only mons, attacks and menus stay "
    .. "above it, and SPRITE LIGHT is forced to UNLIT. OFF fights on the "
    .. "map itself. For a staged fight with no map at all, use 3D-BTL: B.",
    when = function() return stagedBattles() end, full = true },
  ...
  { AntiAlias.setting,
    ...
    full = true },
  -- Not gated on stagedBattles: this one is not part of the battle at all.
  -- It hangs off render.hud, so it is up in the overworld, in menus and in
  -- fights alike, and the row has to be reachable from all three.
  { RunInfo.setting,
    "A solo run's own corner, top right, over everything: the game clock, "
    .. "and beside it two warnings that only appear when they apply -- a "
    .. "bag icon once the bag is down to its last two slots, and the steps "
    .. "left on an active repel. The clock turns gold and stops when the "
    .. "CHAMPION is beaten. Unlike the HUD panels this is drawn over the "
    .. "finished frame rather than into the arena, so it is up while you "
    .. "are walking as well as while you are fighting." },
}
...
mod.hooks:wrap("ui.options.rows", function(next, game, rows)
  ...
  -- ------- no rows of our own -------
  --
  -- Every setting this mod owns lives on the mod manager's page and nowhere
  -- else (see mod.options:define). This hook stays because it still has two
  -- jobs that have nothing to do with our rows: it takes the ENGINE rows we
  -- pin off the list, and it does the pinning. Dropping the hook outright
  -- would put TILT, GBC FX and BATTLE LAYOUT back on the menu while they are
  -- still held at their pinned values -- rows that no longer decide anything,
...
-- ------- the run info corner
--
-- render.hud is the engine's screen-space layer: it fires after
-- Renderer:endFrame and before TouchControls:draw, in LOVE window units, on
-- every frame the game draws. That is what makes this corner always-on --
-- battle, overworld and menus alike -- without either the battle path or the
-- voxel pipeline having to know it exists.
--
-- Wrapped rather than replaced, and the rest of the chain runs FIRST: another
-- mod's status line has as much claim on the frame as this does, and going
-- last is the polite half of sharing a layer. The draw is fenced because a
-- HUD that throws would take the whole frame with it, and a missing clock is
-- a better outcome than a black screen. The error is logged once rather than
-- sixty times a second.
local runInfoErr = nil
mod.hooks:wrap("render.hud", function(next, game, viewport)
  next(game, viewport)
  local ok, err = pcall(RunInfo.draw, game, viewport)
  if not ok and tostring(err) ~= runInfoErr then
    runInfoErr = tostring(err)
    mod.log:warn("run info corner: %s -- it is not drawn this session",
                 runInfoErr)
  end
end)
...
do
  ...
  if not Map.soloBattleBlockHook then
...
do
  ...
  if not OptionsMenu.soloBattleFullHook then
    local Pipelines = require("src.render.Pipelines")
    ...
    OptionsMenu.soloBattleFullHook = true
  end
end
