...
-- A stages the fight on the MAP. B stages it on two carried discs against the
-- sky and draws no map at all (see DiscStage): same camera, same pins, same
-- HUD, because all of that hangs off the arena's cells rather than off what
-- is underneath them.
function OverworldBattle.discMode()
  return OverworldBattle.setting:get() == "discs"
end
...
function OverworldBattle.backPinned()
  ...
  return battle.player and true or false
end
...
--
-- The two blocks trade edges: the foe's goes to the RIGHT of the window and
-- the player's to the LEFT, the opposite of the Game Boy's own arrangement.
-- Set false to put them back.
OverworldBattle.HUD_SIDES_SWAPPED = true

-- How far each block sits from the window edge it is snapped to, in logical
-- pixels. A HUD shake can push a couple of pixels past the edge and they are
-- clipped; that is the whole cost of the snap, and it is invisible.
OverworldBattle.HUD_EDGE_INSET = 2

-- ------- sized and placed in the two free corners
--
-- Where the HUD can go is decided by what else is on screen. The arena camera
-- puts the player's mon low and left (ANCHOR.player) and the foe's high and
-- right (ANCHOR.enemy), and the text box owns the bottom rows of the frame
-- (TEXT_RECT.box). What that leaves free is the TOP-LEFT corner, above the
-- player's own mon, and a band on the RIGHT under the foe's mon and above the
-- box. Those two are where the blocks go.
--
-- An earlier revision fitted them into the gutters beside the text box, which
-- is legible on a 16:9 desktop and not on a 3:2 handheld: at 720x480 the
-- letterbox is 480 wide and the gutter only 120, so the blocks were squeezed to
-- roughly native size in the one place on screen with no room. The corners have
-- room at every aspect because they are measured from the window, not from
-- what the letterbox happened to leave over.

-- How wide the wider of the two blocks should be on screen, and the window
-- width that number was chosen against, so the HUD keeps its relative size on
-- any display rather than being a fixed count that is large on a handheld and
-- small on a 4K panel.
--
-- 470 lands on rung 2 at 720x480 and rung 5 at 1080p. This is the size dial.
OverworldBattle.HUD_TARGET_W = 470
OverworldBattle.HUD_TARGET_REF_W = 1920

-- Whether the scale is held to whole rungs. ON, because a HUD is glyphs before
-- it is anything else: at a whole rung every source pixel is the same square of
-- screen pixels and the strokes stay even, and at 2.13 or 1.2 they do not --
-- some strokes come out a pixel fatter than their neighbours, which on a small
-- panel reads as blur and costs more legibility than the fractional size gains.
OverworldBattle.HUD_PIXEL_SNAP = true

-- How far the foe's block is lifted clear of the top of the text box, in the
-- block's own pixels. It hangs from the box rather than from the foe's mon
-- because the box is the thing it must not collide with.
OverworldBattle.HUD_ENEMY_LIFT = 24

-- The scale the HUD is drawn at.
function OverworldBattle.hudRung(pw)
  local target = OverworldBattle.HUD_TARGET_W
                 * ((pw or 0) / OverworldBattle.HUD_TARGET_REF_W)
  local widest = math.max(OverworldBattle.HUD_RECT.enemy[3],
                          OverworldBattle.HUD_RECT.player[3])
  local rung = target / widest
  -- Rounded rather than truncated: floor turns a target that lands at 1.97 into
  -- rung 1, halving the HUD over a rounding error, and the resolutions that do
  -- that (640x480 among them) are exactly the small ones that can least afford
  -- it.
  if OverworldBattle.HUD_PIXEL_SNAP then rung = math.floor(rung + 0.5) end
...
--
-- Both blocks are measured from the WINDOW's own corners, not from the
-- letterbox, so they sit in the screen's free space rather than in whatever the
-- centred frame left over -- which is the whole point on a panel whose gutters
-- are narrow. A band's own origin is then backed off by its block's offset
-- inside it, so long names, HUD shakes and the intro pokeball rows keep the
-- alignment the engine drew them with and travel with their block.
function OverworldBattle.snapRects(shot)
  ...
  -- a window-scaling HUD) lines up. OG is left alone deliberately -- resizing or
  -- moving it there would break the alignment it exists for, so OG also keeps
  -- the old top/bottom pinning below. SCALED is the mod's default.
  local og = OverworldBattle.hudScaleSetting:get() == "og"
  if og then
    hs = s
  else
    -- One rung below the letterbox remains the ceiling: past that the HUD is
    -- competing with the arena it is supposed to be furniture for.
    hs = math.min(math.max(1, s - 1), OverworldBattle.hudRung(shot.pw))
  end
  local e, p = OverworldBattle.HUD_RECT.enemy, OverworldBattle.HUD_RECT.player
  local eBand = OverworldBattle.HUD_BAND.enemy
  local pBand = OverworldBattle.HUD_BAND.player
  local inset = OverworldBattle.HUD_EDGE_INSET
  local ex, px, eBandY, pBandY, eTop, pTop

  if og then
    -- The original arrangement, untouched: each block hung off its own window
    -- edge, the foe's band by its bottom row and the player's block by its top.
    local pin = { playerTop = 0, enemyBottom = 96 }
    if OverworldBattle.HUD_SIDES_SWAPPED then
      ex = shot.pw - (e[1] + e[3] + inset) * hs
      px = (inset - p[1]) * hs
    else
      ex = (inset - e[1]) * hs
      px = shot.pw - (p[1] + p[3]) * hs
    end
    eBandY = shot.ly + pin.enemyBottom * s - eBand[4] * hs
    pBandY = shot.ly + pin.playerTop * s - (p[2] - pBand[2]) * hs
    eTop = eBandY + (e[2] - eBand[2]) * hs
    pTop = pBandY + (p[2] - pBand[2]) * hs
  else
    local box = OverworldBattle.TEXT_RECT.box
    local margin = inset * hs
    -- The near block takes the top-left corner outright. The foe's takes the
    -- right edge and hangs off the top of the text box, which puts it under the
    -- foe's mon and clear of the box in one measurement.
    local nearX = margin
    local farX = shot.pw - margin - e[3] * hs
    local nearY = margin
    local farY = shot.ly + box[2] * s
                 - OverworldBattle.HUD_ENEMY_LIFT * hs - e[4] * hs

    local eBlockX, pBlockX
    if OverworldBattle.HUD_SIDES_SWAPPED then
      pBlockX, pTop = nearX, nearY
      eBlockX, eTop = farX, farY
    else
      eBlockX, eTop = margin, nearY
      pBlockX = shot.pw - margin - p[3] * hs
      pTop = shot.ly + box[2] * s
             - OverworldBattle.HUD_ENEMY_LIFT * hs - p[4] * hs
    end
    ex = eBlockX - e[1] * hs
    px = pBlockX - p[1] * hs
    eBandY = eTop - (e[2] - eBand[2]) * hs
    pBandY = pTop - (p[2] - pBand[2]) * hs
  end
  local rects = {
    enemy = { ex + e[1] * hs, eTop, e[3] * hs, e[4] * hs },
    player = { px + p[1] * hs, pTop, p[3] * hs, p[4] * hs },
  }
  return rects, {
    enemy = { x = ex, y = eBandY, scale = hs },
    player = { x = px, y = pBandY, scale = hs },
  }
end
...
function OverworldBattle.begin(state, battle)
  ...
  -- A disc rung carries its stage, so it never searches the map and never
  -- declines one.
  local ok, arena
  if OverworldBattle.discMode() then
    ok, arena = pcall(V.require("DiscStage").arena, state.map)
  else
    ok, arena = pcall(BattleArena.find, state.map,
  ...
  if battle then
    battle.soloBattleSurfing = state.player.surfing and true or false
  end
...
function OverworldBattle.sideTexture(battle, side)
  ...
  -- Art authored larger than the 7x7 slot the camera is solved for would stand
  -- that many times too tall. Fit it to the slot in WORLD space, which leaves
  -- every texel of the source on the quad -- the point of supplying a big pic
  -- is resolution, not size. A 192 master then renders on exactly one overworld
  -- tile, the same footprint a vanilla 56 pic has, with 192 texels across it
  -- instead of 56. The guard is native > SLOT_W.front, so 56 art is a no-op and
  -- nothing here needs revisiting for a future size.
  --
  -- Opponent trainer cards are authored for a compact intro slot instead, so
  -- those take their own constant, whichever generation supplied them.
  local presentationScale = 1
  if side == "enemy" and trainer then
    presentationScale = OPPONENT_TRAINER_SCALE
  elseif metric then
    local native = math.max(metric.w, metric.h)
    if native > OverworldBattle.SLOT_W.front then
      presentationScale = OverworldBattle.SLOT_W.front / native
    end
  end
...
function OverworldBattle.refreshSpriteOwnershipHook()
  ...
  BattleState.soloBattleDuplicateFixHook = true
  return true
end

function OverworldBattle.install()
  local OverworldState = require("src.world.OverworldController")
  if not OverworldState.soloBattleBattleHook then
    ...
    OverworldState.soloBattleBattleHook = true
  end
  ...
  if not BattleState.soloBattleTrainerPartyHook then
    ...
    function BattleState.newTrainer(game_, oppClass, partyIndex)
      ...
      battle.soloBattleTrainerParty = partyIndex or 1
      return battle
    end
    BattleState.soloBattleTrainerPartyHook = true
  end
  ...
  if not Renderer.soloBattleBattleFadeHook then
    ...
    Renderer.soloBattleBattleFadeHook = true
  end
  ...
  if BattleState.soloBattleBattleHook then return end
  ...
  if not BattleState.soloBattleEncounterKindHook then
    ...
    function BattleState.newWild(game_, species, level, opts)
      ...
      battle.soloBattleFishing = opts and opts.hooked and true or false
      return battle
    end
    BattleState.soloBattleEncounterKindHook = true
  end
  ...
  function BattleState:draw()
    ...
    if not shot or self.blankForAskName then
      ...
      self.soloBattleShot = nil
      return innerDraw(self)
    end
    self.soloBattleShot = shot
  ...
  function BattleState:drawPicsLayer(slide, sx, sy, onlySide, skipMenuClip)
    local shot = self.soloBattleShot
    if not shot then
  ...
  function BattleState:drawTextArea()
    if not self.soloBattleShot then return innerText(self) end
  ...
  function BattleState:drawZonePass(src, sx, sy)
    if not self.soloBattleShot then return innerZone(self, src, sx, sy) end
  ...
  function BattleState:drawHUDs(slide)
    ...
    if self.soloBattleShot and snapped() then return end
    ...
    if not (self.soloBattleShot and (self.soloBattleDark or color)) then
  ...
  BattleState.soloBattleBattleHook = true
end
...
function OverworldBattle.snapHUDs(battle, shot)
  ...
  local ios = isIOS()
  local panel = UiBackplates.hudUsesPanel()
  local slide = (battle.introSlide or 0) * 4
  local rects, bandPlacement = OverworldBattle.snapRects(shot)
  local enemy, player = OverworldBattle.hudLive(battle, slide)
  local live = {}
  -- Under PANEL nothing is drawn at the engine block's rects, so no glass is
  -- laid there either -- a backplate under a HUD that has been replaced is a
  -- slab floating in the arena. The text box's own rects are unaffected: the
  -- box is still the engine's.
  if enemy and not panel then live.enemy = rects.enemy end
  if player and not panel then live.player = rects.player end
  ...
  -- PANEL draws none of the engine's band, so the scratch-canvas render and
  -- the contrast shader that go into building it are skipped outright rather
  -- than built and thrown away. That is a canvas bind and a full-frame shader
  -- pass per frame off a Mali-G31, which is the difference this row is worth
  -- on the handheld beyond how it looks.
  local layer = nil
  if not panel then
    layer = OverworldBattle.hudTexture(battle, slide, true, color,
                                       colorShadow)
  ...
  local ok, err = pcall(function()
    ...
    if panel then
      ...
      --
      -- The party is resolved BEFORE placing: the foe's box is two pixels
      -- taller when a cluster is going into it, and its bottom edge is what
      -- is pinned to the text box, so its height has to be known first.
      ...
      local at = BattleHudPanel.place(shot,
                                      ...
                                      OverworldBattle.TEXT_RECT.box[2],
                                      party)
      if at then
        if enemy then
...
function OverworldBattle.drawHudPanels(battle)
  local shot = battle.soloBattleShot
  battle.soloBattleDark = nil
