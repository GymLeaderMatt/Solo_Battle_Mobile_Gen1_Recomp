...
-- for free, and persists under options.modOptions.SOLO_BATTLE_VOXEL like the
...
UiBackplates.hudColor = ModSetting.new("hudColor", "HUD COLOR",
  { "COLOR", "INVERTED" }, { "COLOR", "INVERTED" }, 2)
...
-- the shaders above and snapped to the window corner. Its proportions are the
...
-- with a white outline, laid out for this build's 720x480 target, with the
-- name across the full width of the box and a bar that is a fifth of the
-- panel's height rather than a quarter of its status block's.
--
-- PANEL is also the cheaper of the two here, which matters on an H700: OG
-- renders the engine's HUD layer into a scratch canvas every frame so the
-- contrast shader can flip its ink, and PANEL skips that pass entirely.
...
-- flat white field. The illustrated collections (GEN6, PNG) and the boss
-- plate layer they carried are gone -- their job is done by 3D-BTL B's
-- discs and skies, which are geometry rather than a backplate and so cast,
-- receive and hold up under the camera drift.

UiBackplates.arenaFill = ModSetting.new("arenaFill", "ARENA FILL",
  { "OFF", "WHITE" }, { "OFF", "WHITE" }, 1)
...
-- Retained as a seam: nothing is an illustrated plate any more, but the
-- camera and lighting code still asks.
